...
In an SQL shell (e.g. ``psql`` for PostgreSQL), execute the following commands.
Note these are specific to the PostgreSQL dialect, so you may need to change them for e.g. MySQL.
...
.. code-block:: sql
  ...
  CREATE TABLE vcs_releases (
      ...
      event_id uuid,
      record_id uuid,
      record_is_draft boolean DEFAULT FALSE,
...
.. code-block:: bash
  ...
  invenio alembic stamp invenio_vcs@1754318294
